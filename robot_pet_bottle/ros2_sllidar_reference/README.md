# ROS2 SLLidar Reference

라즈베리파이에서 실제로 실행한 아래 명령 기준으로, 필요한 파일과 동작 방식을 보기 쉽게 정리한 폴더입니다.
...
- `slampibot_robot/package.xml`
  - ROS2 의존성 정의
- `slampibot_robot/sdk/README.md`
  - 별도로 준비해야 하는 SLLidar SDK 폴더 구조 설명
...
이 예제의 `sllidar_node.cpp`는 `sl_lidar.h`와 `sdk/src/*.cpp`를 사용합니다. 따라서 `slampibot_robot/sdk/` 아래에 SLLidar SDK가 꼭 있어야 합니다.

정리하면:

- ROS2 패키지들 설치
- `sdk/` 폴더에 SLLidar SDK 소스 준비
...
mkdir -p ~/ros2_ws/src
cd ~/ros2_ws/src
cp -r /path/to/robot_pet_bottle/ros2_sllidar_reference/slampibot_robot .
```

그 다음 `slampibot_robot/sdk/` 안에 SDK를 채운 뒤 빌드합니다.
